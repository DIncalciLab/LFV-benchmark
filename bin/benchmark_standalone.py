...
def load_callers(vcf_path):
    ...
    df_strelka_snv = pd.DataFrame(columns=df_cols)
    df_strelka_indel = pd.DataFrame(columns=df_cols)
    df_strelka = pd.DataFrame(columns=df_cols)
    ...
    for file in vcf_files:
        ...
        if 'vardictjava' in name:
            ...
            for variant in VCF(file):
                if variant.is_snp:
                    if variant.INFO['STATUS'] == 'Germline':
                        continue
                    tmp = pd.DataFrame(data=
                    [
                        [samplename, variant.CHROM,
            ...
            for variant in VCF(file):
                if variant.is_snp:
                    ...
                    [
                        [samplename, variant.CHROM,
                         variant.POS, variant.REF,
                         variant.ALT[0], variant.format('AF')[1][0]
                         ]
                    ], columns=df_cols
                    )
            ...
            if 'indel' in name:
                for variant in VCF(file):
                    ...
                    df_varscan_indel = pd.concat([df_varscan_indel, tmp])
        ...
        elif 'strelka' in name:
            if 'snvs' in name:
                samplename = file.stem.replace('strelka_somatic_', '').split('.')[0]
                for variant in VCF(file):
                    ...
                    df_strelka_snv = pd.concat([df_strelka_snv, tmp])
            elif 'indels' in name:
                samplename = file.stem.replace('strelka_somatic_', '').split('.')[0]
                for variant in VCF(file):
                    ...
                    df_strelka_indel = pd.concat([df_strelka_indel, tmp])
    ...
    df_strelka_snv.set_index('sample')
    df_strelka_indel.set_index('sample')

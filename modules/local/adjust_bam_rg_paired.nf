process ADJUST_BAM_RG_PAIRED {
    tag "Adjust read groups for input BAM: ${meta.sample_name}, test: ${normal.normal_bam}.isEmpty()"
    ...
    output:
    tuple val(meta),           path("*_normal.bam"), path("*_normal.bam.bai"),
                               path("*_tumor.bam"),  path("*_tumor.bam.bai"), optional: true, emit: paired_bam
